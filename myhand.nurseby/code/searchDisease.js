module.exports.function = function searchDisease(Part, Symptom) {
  ...
  for (num in tempResults.part) {
    ...
    for (let i = 0; i < tempResults.coreSymptom.length; i++) {
      console.log(tempResults.part[num].part, "과 검사 : ", tempResults.coreSymptom[i].part)
      if (tempResults.coreSymptom[i].part.includes(tempResults.part[num].part)) {
        findCS = true;
    ...
    for (let i = 0; i < tempResults.symptom.length; i++) {
      console.log(tempResults.part[num].part, "과 검사 : ", tempResults.symptom[i].part)
      if (tempResults.symptom[i].part.includes(tempResults.part[num].part)) {
        findS = true;
        if (results.indexOf(tempResults.part[i]) == -1)
          results.push(tempResults.part[i])
      }
